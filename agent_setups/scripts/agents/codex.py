...
`<host>/ai-gateway/codex/v1` (the Responses wire API) with U2M OAuth.
...
Responses surface
-----------------
Codex sets `wire_api = "responses"`. The `/ai-gateway/codex/v1` route is a unified
Responses surface: the gateway translates, so any model exposing
`mlflow/v1/responses` (a superset of `openai/v1/responses`) is reachable — GPT,
Gemini, Claude, and the open models alike. `--api-type` narrows this if desired.
"""
...
# Codex speaks the Responses wire API. The /ai-gateway/codex/v1 route is a unified
# Responses surface that translates for any responses-capable model, so we filter
# on the broad `mlflow/v1/responses` (superset of `openai/v1/responses`) by default.
DEFAULT_API_TYPE = "mlflow/v1/responses"
NARROW_API_TYPE = "openai/v1/responses"

# Gateway route Codex posts to (the Responses API lives under <base_url>/responses).
DEFAULT_CODEX_PATH = "/ai-gateway/codex/v1"
...
class CodexGenerator(AgentGenerator):
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            ...
            help=(
                ...
                f"Default '{DEFAULT_API_TYPE}' (the broad Responses surface — the "
                "/codex/v1 route translates, so this covers GPT, Gemini, Claude, and open "
                f"models). Narrow to '{NARROW_API_TYPE}' for OpenAI-native responses only."
            ),
        )
        parser.add_argument(
            "--skip-api-discovery",
        ...
        parser.add_argument(
            ...
            help=f"Gateway route base appended to the host for base_url (default: {DEFAULT_CODEX_PATH}).",
        )
        parser.add_argument(
            "--refresh-interval-ms",
            type=int,
    ...
    def generate(self, ctx: GatewayContext, args: argparse.Namespace) -> dict[str, str]:
        ...
        base_url = f"{ctx.host}{args.codex_path}"
        ...
        lines = [
            ...
            f"# ({args.codex_path}, wire_api=\"responses\"). Gateway routing + OAuth only — the",
