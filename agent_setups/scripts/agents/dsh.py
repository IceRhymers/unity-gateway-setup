...
-----------------------
  ...
  - Auth : the adapter resolves its `apiKeyEnv` reference per request through
    ...
    Databricks CLI and writes it to that reference on a timer. This gives the
    same per-request freshness as Claude Code's `apiKeyHelper`, but through
    DSH's credential seam.
...
# `<baseURL>/chat/completions`, so it needs the mlflow base.
...
// session. This gives per-request freshness through DSH's credential seam.
//
