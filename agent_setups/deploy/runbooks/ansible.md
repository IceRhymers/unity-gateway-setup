...
| Code | Meaning |
|---|---|
| 0 | Success (or `--dry-run` / `--print-target-dir` / `--uninstall` with all files removed) |
| 1 | Usage error |
...
- **Idempotency:** `install.sh` copies a file only when its content changed.
  When the bundle is unchanged, `install.sh` prints `unchanged:` for each file.
  It backs up the previous file as `.bak-<UTC>` before overwriting.
  It always refreshes the version marker.
  You can safely re-run the play on an already-configured machine.
- **Uninstall:** pass `--uninstall` to remove the files that `install.sh` placed.
  Uninstall reads the `files=` list from the version marker.
