...
2. `model-service` (fanned out with `for_each`) — an FMAPI-backed model service
   for every endpoint in the provider catalog. Each service has inference logging
   enabled, so it captures traffic to a UC Delta table.
3. `telemetry` (when `telemetry_enabled = true`, the default) — the OpenTelemetry
   ingestion stack. This stack has a `telemetry` schema, the OTEL
   metrics/logs/traces Delta tables, a Databricks-managed service principal +
   workspace OAuth secret, a `databricks_secret_uc` that holds those credentials,
   and the grants. Coding agents emit OTLP here. See
   [modules/telemetry](../modules/telemetry/README.md).
...
- **versioned_models** — version-pinned endpoints whose name is the `system.ai`
  ...
  `claude-opus-4-8`). These serve power users who pin a version and harnesses
...
SQL warehouse** to create the tables. Terraform resolves the warehouse by name
via `telemetry_warehouse_name` (default `"Serverless Starter Warehouse"`, which
auto-starts). Set `telemetry_warehouse_id` to skip the lookup. To grant
developers read access to the credential, list their group in
`telemetry_reader_groups`, or grant `READ_SECRET` on the UC secret out of band.
After `apply`, regenerate the Claude Code settings with `make agent-claude-code`.
The generator reads the `telemetry` output and adds the OTEL env +
`otelHeadersHelper`. Set `telemetry_enabled = false` to skip it.
